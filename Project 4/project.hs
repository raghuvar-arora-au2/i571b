...
deptEmployees [] dept =  []
deptEmployees (emp:empTail) dept   
  | (emplDept emp) == dept = emp : deptEmployees empTail dept 
  | otherwise = deptEmployees empTail dept
...
comprDeptEmployees emps dept = 
    [x| x<-emps,(emplDept x) == dept]
...
employeesSalarySum emps = 
  foldr (\emp acc -> (emplSalary emp)+acc) 0 emps
...
charIndexes s c = 
    -- filter (\(a,b) ->  b == c)
    -- zip [0..length s] s
    -- let zipped= (zip [0..length s] s) in
    [ index| (index, element)<-(zip [0..length s] s) , element == c]
...
foldTree treeFn leafFn (Leaf a) = 
    leafFn a
foldTree treeFn leafFn (Tree  left a  right) =
    treeFn (foldTree treeFn leafFn left)  a   (foldTree treeFn leafFn right)
